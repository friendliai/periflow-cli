...
class ExperimentClientService(ClientService):
    def list_jobs_in_experiment(self, experiment_id: T) -> List[dict]:
        response = safe_request(
            self.list, err_prefix="Failed to fetch jobs in the experiment."
    ...
    def update_experiment_name(self, experiment_id: T, name: str) -> dict:
        response = safe_request(
            self.partial_update,
            err_prefix=f"Failed to update the name of experiment to {name}",
        )(pk=experiment_id, json={"name": name})
