...
class JobConfigManager:
    ...
    def from_file(cls, f: _IO) -> JobConfigManager:
        ...
        abs_class = cls(config)
        abs_class.validate()
        job_type = config["job_setting"]["type"]

        if job_type == "custom":
            return CustomJobConfigManager(config)
        else:
            return PredefinedJobConfigManager(config)
...
class CustomJobConfigManager(JobConfigManager):
    """CustomJob Configuration manager"""
    ...
    def get_job_request_body(self) -> Dict[str, Any]:
        ...
        if "workspace" not in body["job_setting"]:
...
class PredefinedJobConfigManager(JobConfigManager):
