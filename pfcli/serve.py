...
def list(
    tail: Optional[int] = typer.Option(
        None, "--tail", help="The number of serve list to view at the tail"
    ),
    head: Optional[int] = typer.Option(
        None, "--head", help="The number of serve list to view at the head"
    ),
):
    """List all serves,"""
...
@app.command()
def stop(serve_id: str = typer.Argument(..., help="ID of serve to stop")):
    """Delete serve."""
    client: ServeClientService = build_client(ServiceType.SERVE)
...
@app.command()
def view(serve_id: str = typer.Argument(..., help="serve id to inspect detail.")):
...
def create(
    ...
    gpu_type: GpuType = typer.Option(
        ..., "--gpu-type", "-g", help="The GPU type where the serve is deployed."
    ),
    num_sessions: int = typer.Option(
        ..., "--num-sessions", "-s", help="The number of sessions of serve deployment."
    ),
):
