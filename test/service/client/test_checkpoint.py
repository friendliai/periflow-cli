...
from pfcli.utils.fs import S3_MPU_PART_MAX_SIZE, S3_UPLOAD_SIZE_LIMIT
...
@pytest.fixture
def checkpoint_form_client() -> CheckpointFormClientService:
    return build_client(ServiceType.CHECKPOINT_FORM)
...
def test_checkpoint_client_get_checkpoint_download_urls(
    ...
    assert isinstance(checkpoint_form_client, CheckpointFormClientService)
    ckpt_form_id = uuid4()
...
def test_actual_s3_upload(
    requests_mock: requests_mock.Mocker, checkpoint_form_client: CheckpointClientService
):
    assert isinstance(checkpoint_form_client, CheckpointFormClientService)
    ckpt_form_id = uuid4()
