...
def test_experiment_client_list_jobs_in_experiment(
    requests_mock: requests_mock.Mocker, experiment_client: ExperimentClientService
):
    ...
    requests_mock.get(
        url_template.render(experiment_id=1), json={"results": [{"id": 0}, {"id": 1}]}
    )
...
@pytest.mark.usefixtures("patch_auto_token_refresh")
def test_experiment_client_delete_experiment(
    requests_mock: requests_mock.Mocker, experiment_client: ExperimentClientService
):
    assert isinstance(experiment_client, ExperimentClientService)
...
@pytest.mark.usefixtures("patch_auto_token_refresh")
def test_experiment_client_update_experiment(
    requests_mock: requests_mock.Mocker, experiment_client: ExperimentClientService
):
    assert isinstance(experiment_client, ExperimentClientService)
    ...
    url_template.attach_pattern("$experiment_id/")
    requests_mock.patch(
        url_template.render(experiment_id=1), json={"id": 0, "name": "my-exp"}
    )
    assert experiment_client.update_experiment_name(1, "my-exp") == {
        "id": 0,
        "name": "my-exp",
    }
