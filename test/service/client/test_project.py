...
@pytest.mark.usefixtures('patch_auto_token_refresh')
def test_project_job_client_run_job(requests_mock: requests_mock.Mocker,
                                    project_job_client: ProjectJobClientService):
    ...
    # Failed due to HTTP error
    requests_mock.post(
        project_job_client.url_template.render(**project_job_client.url_kwargs), status_code=500
    )
    with pytest.raises(typer.Exit):
